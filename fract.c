...
/*   Updated: 2025/01/04 15:45:28 by yalrfai          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	see_iteration(double c_re, double c_im, double z_re, double z_im)
{
	...
	iteration = 0;
	while (z_re * z_re + z_im * z_im < 4 && iteration < MAX_ITR)
	{
...
void	mandelbrot(t_mlx_data *data)
{
	...
	while (++y < HEIGHT)
	{
		...
		while (++x < WIDTH)
		{
			c_re = map(x, WIDTH, -2.5 * data->pic.zoom + data->pic.offsetx,
					1.5 * data->pic.zoom + data->pic.offsetx);
			c_im = map(y, HEIGHT, -2 * data->pic.zoom + data->pic.offsety,
...
void	julia(t_mlx_data *data, double c_re, double c_im)
{
	...
	while (++y < HEIGHT)
	{
		...
		while (++x < WIDTH)
		{
			z_re = map(x, WIDTH, -2.5 * data->pic.zoom + data->pic.offsetx,
					1.5 * data->pic.zoom + data->pic.offsetx);
			z_im = map(y, HEIGHT, -2 * data->pic.zoom + data->pic.offsety,
					2 * data->pic.zoom + data->pic.offsety);
